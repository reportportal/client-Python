...
def _get_id(response):
    try:
        ...
        raise EntryCreatedError("No 'id' in response: {0}".format(response.text))
...
def _get_msg(response):
    try:
        ...
        raise OperationCompletionError("No 'msg' in response: {0}".format(response.text))
...
def _get_json(response):
    try:
        ...
        raise ResponseError("Invalid response: {0}: {1}".format(value_error, response.text))
    ...
    for ret in data.get("responses", [data]):
        if "message" in ret:
            if "error_code" in ret:
                error_messages.append("{0}: {1}".format(ret["error_code"], ret["message"]))
            else:
