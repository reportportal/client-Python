...
@pytest.mark.parametrize(
    ...
    [((15, 17), 15, 17), (21, 21, 21), (None, None, None)],
)
...
async def test_verify_ssl_str(connector_mock: mock.Mock):
    client = Client(
        "http://endpoint", "project", api_key="api_key", verify_ssl=os.path.join(os.getcwd(), "test_res/root.pem")
    )
    await client.session()
    connector_mock.assert_called_once()
    _, kwargs = connector_mock.call_args_list[0]
...
async def test_keepalive_timeout(connector_mock: mock.Mock):
    ...
    client = Client("http://endpoint", "project", api_key="api_key", keepalive_timeout=keepalive_timeout)
    await client.session()
    connector_mock.assert_called_once()
    _, kwargs = connector_mock.call_args_list[0]
...
async def test_start_launch(aio_client: Client):
    ...
    result = await aio_client.start_launch(
        launch_name, start_time, description=description, attributes=attributes, rerun=True, rerun_of=rerun_of
    )
...
@pytest.mark.asyncio
async def test_connection_errors(aio_client, requests_method, client_method, client_params):
...
async def test_start_test_item(aio_client: Client, parent_id, expected_uri):
    ...
    result = await aio_client.start_test_item(
        launch_uuid,
        item_name,
        start_time,
        item_type,
...
async def test_finish_test_item(aio_client: Client):
    ...
    result = await aio_client.finish_test_item(
        launch_uuid,
        item_id,
        end_time,
        status=status,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("start_test_item", mock_basic_post_response, "post", ["test_launch_uuid", "Test Item", timestamp(), "SUITE"]),
        ("finish_test_item", mock_basic_post_response, "put", ["test_launch_uuid", "test_item_uuid", timestamp()]),
