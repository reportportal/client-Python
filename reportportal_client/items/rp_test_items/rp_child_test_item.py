...
from reportportal_client.items.rp_test_items.rp_base_test_item import RPBaseTestItem
...
class RPChildTestItem(RPBaseTestItem):
    ...
    def __init__(self, rp_url, session, api_version, project_name, parent_item, item_name, item_type, launch_uuid,
        ...
        super(RPChildTestItem, self).__init__(rp_url, session, api_version, project_name, item_name, item_type,
    ...
    def start(self, start_time):
        ...
        """
        endpoint = "{rp_url}/{api_version}/{project_name}/item/{parentItemUuid}".format(rp_url=self.rp_url,
                                                                                        api_version=self.api_version,
                                                                                        project_name=self.project_name,
                                                                                        parentItemUuid=self.parent_item.uuid)

        self.add_request(endpoint, self.session.post, ItemStartRequest, self.item_name, start_time,
                         self.item_type, self.launch_uuid, attributes=self.attributes, code_ref=self.code_ref,
                         description=self.description, has_stats=self.has_stats, parameters=self.parameters,
                         retry=self.retry, uuid=self.uuid, unique_id=self.unique_id)
