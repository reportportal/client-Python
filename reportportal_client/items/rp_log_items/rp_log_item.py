...
class RPLogItem(BaseRPItem):
    ...
    def __init__(self, rp_url, session, api_version, project_name, launch_uuid):
        ...
        """
        super(RPLogItem, self).__init__(rp_url, session, api_version, project_name, launch_uuid)
        self.weight = 1
    ...
    def create(self, time, file_obj=None, item_uuid=None, level=RP_LOG_LEVELS[40000], message=None):
        """
        ...
        """
        endpoint = "/api/{version}/{projectName}/log".format(version=self.api_version, projectName=self.project_name)
        self.add_request(endpoint, self.session.post, RPRequestLog, self.launch_uuid, time, file=file_obj,
